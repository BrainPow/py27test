...
class fullMenu:
    ...
    def open(this):
        if _time.time() - this.client.CMDTime > 1:
            ...
            this.client.close()
            this.client.room.addTextArea(10002, "<img src='https://i.hizliresim.com/VMDVyV.png'>", this.client.playerName, 70, 20, 670, 500, 0, 0, 0, False)
...
class roleta:
    ...
    def open(this):
        if _time.time() - this.client.CMDTime > 1:
            ...
            this.client.room.addTextArea(10004, this.client.getText("roleta.main", this.client.playerName, this.client.XDFichas), this.client.playerName, 200, 95, 230, 220, 0, 0, 0, False)
            this.client.getFull()
            this.sendRoleta()

    def sendRoleta(this):
        if this.client.XDFichas >=10:
            tam = 80
        else: tam = 70
        this.sendRemovePoup(63)
        this.client.room.addTextArea(10050, "<img src='https://i.hizliresim.com/pWngYL.png'>", this.client.playerName, 420, 85, 670, 500, 0, 0, 0, False)
    ...
    def sendGetTimeRoleta(this):
        ...
        else: this.client.sendMessage("<N>[Bilgi] <J>Ödül için bekleyiniz...")

    def sendSorteioRoleta(this):
        if _time.time() - this.client.CMDTime > 1:
            this.client.CMDTime = _time.time()
            if this.client.TimeGiro == 1:
                lista = ["first", "morangos", "firsts", "moedas", "queijos na loja", "morangos na loja", "queijo coletado", "bootcamp", "first", "moedas", "queijo coletado", "morangos", "moedas", "morangos", "moedas", "queijo coletado"]
                rkey = str(random.choice(lista))
                this.sendRemovePoup(62)
                this.client.XDFichas -= 1
                this.client.Cursor.execute('UPDATE Users SET XDFichas = ? WHERE Username = ?', [this.client.XDFichas, this.client.playerName])
                this.client.Cursor.fetchone()
                if rkey == "first":
                        listaQuant = ["3", "5", "7", "2", "1", "9", "10", "4"]
                elif rkey == "queijo coletado":
                        listaQuant = ["3", "5", "7", "2", "1", "9", "10", "4"]
                ...
                if rkey == "queijos na loja":
                        ...
                        this.client.XDCoins += int(Quant)
                        this.client.Cursor.execute('UPDATE Users SET XDCoins = ? WHERE Username = ?', [this.client.XDCoins, this.client.playerName])
                        this.client.Cursor.fetchone()
                elif rkey == "fichas":
                        this.client.XDFichas += int(Quant)
                        this.client.Cursor.execute('UPDATE Users SET XDFichas = ? WHERE Username = ?', [this.client.XDFichas, this.client.playerName])
                        this.client.Cursor.fetchone()
                            
                bg = '<img src="https://i.hizliresim.com/ERDYpA.png">'
                txt = 'Parabéns, você ganhou '+str(Quant)+' '+str(rkey)+'.'
                txtOK = '<font size="12"><V><a href="event:fecharPop">TAMAM</a></font>'
                msg = "<VP>Tebrikler kazancınız <N> "+str(Quant)+" "+str(rkey)+" <VP>hesabınıza eklendi."
...
class equipe:
    ...
    def open(this):
        this.client.close()
        this.client.room.addTextArea(10002, "<img src='https://i.hizliresim.com/XX6vLk.png'>", this.client.playerName, 70, 20, 670, 500, 0, 0, 0, False)
    ...
    def sendStaff(this, type):
        if _time.time() - this.client.CMDTime > 1:
            this.client.CMDTime = _time.time()
            lists = ["<p align='left'>", "<p align='left'>", "<p align='left'>", "<p align='left'>", "<p align='left'>", "<p align='left'>", "<p align='left'>", "<p align='left'>"]
            if type == 0:
                this.client.Cursor.execute("select Username, PrivLevel from Users where PrivLevel = 10 Order by PlayerID LIMIT 7")
            if type == 1:
            ...
            for rs in r:
                ...
                lists[{11:0, 10:1, 9:2, 8:3, 7:4, 6:5, 5:6, 4:7}[privLevel]] += "\n" + " <V> " +playerName+ "<N> - <V>" + {10: "<ROSE>Administrador", 9:"<VI>Coordenador", 8:"<J>Super Moderador", 7:"<N>Moderador", 6:"<font color='#FFCC99'>MapCrew", 5:"<font color='#3366FF'>Helper</font>", 4:"<font color='#00FA9A'><a href='http://www.transforvida.com.br/lojinha/' target='_blank'>Premium</a></font>"}[privLevel] + "<V><N> - " + ("<N>[<VP>Online<N>] \n <N> " if this.server.checkConnectedAccount(playerName) else "<N>[<R>Offline<N>] \n")
            if type == 0:
                this.open()
                this.client.room.addTextArea(10014, "<p align = \"left\"><font size = \"13\">  Cargo que exerce . funções.\n" + "".join(lists) + "</p>""<br><br></p>", this.client.playerName, 400, 96, 470, 215, 0, 0, 0, False)
            if type == 1:
...
class ranking:
    ...
    def sendRanking(this, type):
        if _time.time() - this.client.CMDTime > 1:
            ...
            if type == 1:
                this.open()
                names = "<font size='12'><ROSE>Usuário</ROSE></font>\n"
            ...
            if type == 2:
                names = "<font size='12'><ROSE>Usuário</ROSE></font>\n"
                counts = "<font size='12'><ROSE>Queijo</ROSE></font>\n"
...
class shop:
    ...
    def open(this):
        if _time.time() - this.client.CMDTime > 1:
            ...
            this.client.room.addTextArea(10004, this.client.getText("shop.main", this.client.playerName, this.client.XDCoins), this.client.playerName, 200, 95, 495, 380, 0, 0, 0, False)
            this.client.getFull()
    ...
    def changeTab(this, tab, page):
        if _time.time() - this.client.CMDTime > 1:
            ...
            this.client.room.addTextArea(10014, this.client.getText("shop.main", this.client.playerName, this.client.XDCoins), this.client.playerName, 200, 95, 495, 380, 0, 0, 0, False)
            text = this.client.getText("shop.main" if tab == 1 else "shop.header", this.client.playerName, this.client.XDCoins)
                ...
                if type == 1:
                    ...
                    if id == 1:
                        if not this.client.privLevel >= 2:
                            ...
                            this.client.XDCoins += 500
                            bg = '<img src="https://i.hizliresim.com/pWngXo.png">'
                            txt = '<p align="center"><N>Parabéns! Você comprou <ROSE>VIP<N> no <ROSE>HideMice <N>por <ROSE>30 dias<N> e ganhou também <ROSE>500<N> moedas e firsts.</p>'
                            ...
                            this.client.sendMessage("<ROSE>Digite /ajudavip para ver seus privilégios.")
                            this.client.updateDatabase()
                        else:
                            this.client.XDCoins += 5000
                            this.client.sendMessage("<ROSE>Você já possui VIP ou um cargo privilegiado no servidor.")
                    elif id in [2, 3, 4, 5]:
                        bg = '<img src="https://i.hizliresim.com/pWngXo.png">'
                        txt = '<p align="center">asdsa</p>'
                        txtOK = '<font size="12"><V><a href="event:fecharPop">TAMAM</a></font>'
                        this.client.sendAddPopupText(10056, 300, 130, 299, 100, '000000', '000000', 100, bg)
                        this.client.sendAddPopupText(10057, 325, 150, 225, 60, '000000', '000000', 100, txt)
                    ...
                    this.client.XDFichas += (10 if id == 1 else 20 if id == 2 else 30 if id == 3 else 40 if id == 4 else 50 if id == 5 else 0)
...
class consumablesShop:
    ...
    def open(this):
        if _time.time() - this.client.CMDTime > 1:
            ...
            while slot < 6:
                ...
                this.client.room.addTextArea(10040 + slot, "<img src='http://www.tfmflash.com/api/consumablesShop/getItem.php?cheeses=%s&fraises=%s&id=%s' hspace='-2' vspace='0'>" %(consumable[1], consumable[2], consumable[0]), this.client.playerName, (200 + (140 * slot)) - (420 * (slot / 3)), 95 + (110 * (slot / 3)), 130, 105, 0, 0, 0, False)
    ...
    def itemsPage(this):
        ...
        this.client.room.addTextArea(10057, "<img src='http://www.transforvida.com.br/api/consumablesShop/price.png' hspace='0' vspace='0'>", this.client.playerName, 635, 170, 20, 45, 0x000001, 0x000001, 0, False)
        ...
        if _time.time() - this.client.CMDTime > 1:
            ...
            while slot < ((page - 1) * 6) + 6:
                if slot >= len(consumables):
                    break
                consumable = consumables[str(slot)]
                this.client.room.addTextArea(10040 + (slot % 6), "<img src='http://www.tfmflash.com/api/consumablesShop/getItem.php?cheeses=%s&fraises=%s&id=%s' hspace='-2' vspace='0'>" %(consumable[1], consumable[2], consumable[0]), this.client.playerName, (200 + (140 * (slot % 6))) - (420 * ((slot % 6) / 3)), 95 + (110 * ((slot % 6) / 3)), 130, 105, 0, 0, 0, False)
    ...
    def endBuy(this):
        if _time.time() - this.client.CMDTime > 1:
            ...
            if consumablesCount > 0:
                ...
                this.client.room.addTextArea(10061, ("<a href='event:consumablesShop:confirmBuy-0'>" if this.client.shopCheeses >= this.cheesesCount else "") + "<img src='http://www.tfmflash.com/api/consumablesShop/getPrice.php?type=0&value=%s&can=%s' hspace='0' vspace='-2'>" %(this.cheesesCount, ("1" if this.client.shopCheeses >= this.cheesesCount else "0")) + ("</a>" if this.client.shopCheeses >= this.cheesesCount else ""), this.client.playerName, 280, 190, 120, 60, 0, 0, 0, False)
                this.client.room.addTextArea(10062, ("<a href='event:consumablesShop:confirmBuy-1'>" if this.client.shopFraises >= this.fraisesCount else "") + "<img src='http://www.tfmflash.com/api/consumablesShop/getPrice.php?type=1&value=%s&can=%s' hspace='0' vspace='-2'>" %(this.fraisesCount, ("1" if this.client.shopFraises >= this.fraisesCount else "0")) + ("</a>" if this.client.shopFraises >= this.fraisesCount else ""), this.client.playerName, 480, 190, 120, 60, 0, 0, 0, False)
