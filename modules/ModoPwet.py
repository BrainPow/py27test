...
class ModoPwet:
    ...
    def makeReport(this, playerName, type, comments):
        ...
        this.server.sendStaffMessage(7, "<V>%s</V> está reportando o jogador <R>%s</R> por <J>%s</J> pelo seguinte motivo: <J>%s</J>" %(this.client.playerName, playerName, {0:"Hack", 1:"Spam / Flood", 2:"Insultos", 3:"Pishing", 4:"Outro"}[type], "-" if comments == "" else comments))
        ...
        if len(this.server.reports["names"]) <= 0:
            ...
            while reports < totalReports:
                ...
                if this.client.modoPwetLangue == "ALL" or this.server.reports[playerName]["langue"] == this.client.modoPwetLangue.upper():
                    ...
                    packet.writeUnsignedByte(count).writeUTF(this.server.reports[playerName]["langue"].upper()).writeUTF(playerName).writeUTF(this.getPlayerRoomName(playerName)).writeInt(this.getProfileCheeseCount(playerName))
