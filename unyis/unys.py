...
class Client:
    ...
    def dataReceived(this, packet):
        #print "\nPacket: %s - Jogador: %s - Sala: %s" % (repr(packet), this.playerName, this.roomName)
        ...
        if packet == None or len(packet) < 2:
            ...
            while this.packages.strip(chr(0)):
                if len(this.packages) >= 5:
                    ...
                    packetLength = p.readUnsignedByte() if packetFormat == 1 else p.readUnsignedShort() if packetFormat == 2 else ((p.readUnsignedByte() & 0xFF) << 16) | ((p.readUnsignedByte() & 0xFF) << 8) | (p.readUnsignedByte() & 0xFF) if packetFormat == 3 else 0
    ...
    def protectFirewall(this):
        ...
        this.server.sendModMessage(4, "[<J>"+this.ipAddress+"<BL>] Ataque Detectado - IP Banido no Firewall/AntiDDos!")
    ...
    def makeConnection(this, transport):
        ...
        else: this.server.connectedCounts[this.ipAddress] = 1
    ...
    def connectionLost(this, args):
        ...
        if this.server.players.has_key(this.playerName):
            ...
            if this.privLevel >= 4:
                this.server.sendStaffMessage(4, "<ROSE>[%s][%s] <CH>%s <N>desconectou-se." %({11:"Admin", 10:"Admin", 9:"Coord", 8:"Smod", 7:"Mod", 6:"MapCrew", 5:"Helper", 4:"Premium"}[this.privLevel], this.langue, this.playerName), True)

            this.lastOn = this.tribulle.getTime()
            this.tribulle.friendChanged = True
            for player in this.server.players.values():
                if not this.tribeCode in ["", None, 0, "0"]:
    ...
    def parseString(this, packet):
        ...
        if not this.validatingVersion:
            if (C == Identifiers.recv.Informations.C and CC == Identifiers.recv.Informations.Correct_Version) and not (this.isClosed):
                ...
                else: this.transport.loseConnection()                
        else:
            try:
                if packet == "" or packet == " ":
                    this.server.tempIPBanList.append(this.ipAddress)
                ...
                with open("./SErrors.log", "a") as f:
                    ...
                    c.close()
                    this.server.sendModMessage(7, "<BL>[<R>ERROR<BL>] O usuário <R>%s achou um erro no sistema." %(this.playerName))
                    this.gameError += 1
                    if this.gameError >= 5:
                        this.sendPlayerBan(0, "Você foi desconectado por erros no sistema, reentre no jogo.", True)

    def resultLimitPacket(this):
        this.transport.loseConnection()
        this.server.disconnectIPAddress(this.ipAddress)
        this.server.sendModMessage(7, "<N>[<R>Atividade Suspeita<N>] O usuário <ROSE>%s<N> foi expulso por Limit Packet." %(this.playerName))
    ...
    def sendStaffChats(this):
        if this.privLevel >= 4:
            text = ""
            clear = "<a href='event:clearStaffChat'>  Limpar</a>"
            off = "<a href='event:desativarStaffChat'>Desativar</a>"
            on = "<a href='event:desativarStaffChat'> Reativar</a>"
            ...
            for message in this.server.staffChat:
                if not this.server.activeStaffChat == 1:
                    ...
                    d = date.today()
                    text = "<font size='13'><N>O chat foi desativado pelo Administrador <V>"+str(this.playerName)+"<N> em <V>"+str(d.day)+"/"+str(d.month)+"/"+str(d.year)+"<N>.</font><BR>"
            ...
            if this.openStaffChat:
                if this.privLevel >= 9:
                    if len(this.server.staffChat) >= 1:
                        ...
                        else: this.sendMBox(on, 670, 95, 60, 20, "100%", "212E35", "212E35", 6)
    ...
    def loginPlayer(this, playerName, password, startRoom):
        ...
        if this.server.checkConnectedAccount(playerName):
            ...
            if this.privLevel == -1:
                this.sendPacket(Identifiers.old.send.Player_Ban_Login, ["Conta bloqueada."])
                this.transport.loseConnection()
                return
            ...
            if not this.tribeCode in ["", "None", None, 0]:
                this.getTribeInfo()
            ...
            this.reactorCheckPacket()
            #reactor.callLater(50, this.musicName)
            if this.cheeseCount >= 1002:
                this.AwardsPlayers.sendpremiohours()
                this.AwardsPlayers.sendAdsense()
            reactor.callLater(0.5, this.sendLoginMessage)
            if this.privLevel == 2:
                this.sendMessage("Digite: <ROSE>/ajudavip<BL> para ver os comandos <ROSE>VIP<BL>.")
                this.sendMessage("Sistema de Eleição funcionando, veja no Menu e participe!")
                this.checkVip(vipTime)

    def sendPlayRadio(this):
         this.sendMessage("<R>Você está ouvindo a nossa rádio. Para desligar digite <b>/stop</b>.")
...
##    def musicName(this):
##        music = urllib.urlopen("http://radioazzy.com/api/atualmusica")
##        contents = music.read()
##        music.close()
##        this.sendMessage("<V>Tocando agora na rádio: <BV>"+contents+"")
##        reactor.callLater(200, this.musicName)

    def sendLoginMessage(this):
        if this.privLevel >= 4:
            this.server.sendStaffMessage(4, "<ROSE>[%s][%s] <CH>%s <N>conectou-se." %({11:"Admin", 10:"Admin", 9:"Coord", 8:"Smod", 7:"Mod", 6:"MapCrew", 5:"Helper", 4:"Premium"}[this.privLevel], this.langue, this.playerName), True)
            this.sendMessageLogin()
            this.sendRulesMessage()
            
        if this.privLevel <= 3 and not this.cheeseCount == 1000:
            this.server.sendModMessage(4, "[<J>"+this.ipAddress+"<BL>] O jogador <V>"+this.playerName+"<BL> conectou-se no servidor.")
        
        if this.privLevel >= 1:
            this.sendMessage("<N>Olá <J>"+str(this.playerName)+"<N>. Use <J>/comandos<N> para visualizar nosso menu.")
            this.sendPlayRadio()

    def sendMessageLogin(this):
        this.sendMessage('<VP>Servidor online há: '+str(datetime.today() - this.server.STARTTIME).replace('<', '&lt;').split('.')[0]+' horas.')

    def sendRulesMessage(this):
        this.sendMessage("<VP>Atenção caro STAFF, veja as regras para não perder seu respectivo cargo. Digite: /rules.\nPara abrir o chat da equipe, aperte a letra J.")
    ...
    def retime(this):
        if this.privLevel >= 3:
            ...
            this.sendMessage("<V>Você reviveu sem limites por ser da <ROSE>Equipe<V> ^_^")
        else:
            if int(this.getTimer() - this.room.gameStartTime) > 35:
                this.sendMessage("<V>Você poderá reviver em: <ROSE>"+str(this.room.roundTime + this.room.addTime + int(this.room.gameStartTime - this.getTimer()))+" <V>segundos! ^_^")
                this.sendMessage("Você tem 35 segundos para reviver em cada novo mapa iniciado.")
            else:
                this.room.respawnSpecific(this.playerName)
                this.sendMessage("<V>Você reviveu nos primeiros <ROSE>35<V> segundos do mapa ^_^")  
    ...
    def updateDatabase(this):
        ...
        Cursor.execute("update Users set PrivLevel = ?, ShamanSaves = ?, ShamanCheeses = ?, FirstCount = ?, CheeseCount = ?, TitleNumber = ?, ShopItems = ?, FriendsList = ?, Look = ?, ShopCheeses = ?, CheeseTitleList = ?, FirstTitleList = ?, ShamanTitleList = ?, ShopTitleList = ?, HardModeSaves = ?, HardModeTitleList = ?, Email = ?, MouseColor = ?, BootcampCount = ?, BootcampTitleList = ?, ShopFraises = ?, RegDate = ?, XDCoins = ?, Skills = ?, LastOn = ?, IgnoredsList = ?, Gender = ?, Marriage = ?, LastDivorceTimer = ?, DivineModeSaves = ?, DivineModeTitleList = ?, Badges = ?, XDFichas = ?, ShamanBadges = ?, EquipedShamanBadge = ?, EquipedConsumables = ?, DeathCount = ?, SurvivorStats = ?, RacingStats = ?, Consumables = ?, BanHours = ?, Clothes = ?, ShamanType = ?, ShamanItems = ?, ShamanColor = ?, ShamanLook = ?, ShamanLevel = ?, ShamanExp = ?, ShamanExpNext = ?, TribeCode = ?, TribeRank = ?, TribeJoined = ?, Pet = ?, PetEnd = ?, AventureCounts = ?, AventurePoints = ?, SavesAventure = ?, deathStats = ?, DeathRounds = ?, VisuDone = ?, SpecialTitleList = ?, Mayor = ? where Username = ?", [this.privLevel, this.shamanSaves, this.shamanCheeses, this.firstCount, this.cheeseCount, this.titleNumber, this.shopItems, str(this.friendsList), this.playerLook, this.shopCheeses, ",".join(map(str, this.cheeseTitleList)), ",".join(map(str, this.firstTitleList)), ",".join(map(str, this.shamanTitleList)), ",".join(map(str, this.shopTitleList)), this.hardModeSaves, ",".join(map(str, this.hardModeTitleList)), this.emailAddress, this.mouseColor, this.bootcampCount, ",".join(map(str, this.bootcampTitleList)), this.shopFraises, this.regDate, this.XDCoins, ";".join(map(lambda skill: "%s:%s" %(skill[0], skill[1]), this.playerSkills.items())), this.tribulle.getTime(), str(this.ignoredsList), this.gender, this.marriage, this.lastDivorceTimer, this.divineModeSaves, ",".join(map(str, this.divineModeTitleList)), ",".join(map(str, this.shopBadges)), this.XDFichas, ",".join(map(str, this.shamanBadges)), this.equipedShamanBadge, ",".join(map(str, this.equipedConsumables)), this.deathCount, ",".join(map(str, this.survivorStats)), ",".join(map(str, this.racingStats)), ";".join(map(lambda consumable: "%s:%s" %(consumable[0], 250 if consumable[1] > 250 else consumable[1]), this.playerConsumables.items())), this.banHours, "|".join(map(str, this.clothes)), this.shamanType, this.shamanItems, this.shamanColor, this.shamanLook, this.shamanLevel, this.shamanExp, this.shamanExpNext, this.tribeCode, this.tribeRank, this.tribeJoined, this.pet, abs(Utils.getSecondsDiff(this.petEnd)), ";".join(map(lambda aventure: "%s:%s" %(aventure[0], aventure[1]), this.aventureCounts.items())), ";".join(map(lambda points: "%s:%s" %(points[0], points[1]), this.aventurePoints.items())), this.aventureSaves, ",".join(map(str, this.deathStats)), this.deathRounds, "|".join(map(str, this.visuDone)), ",".join(map(str, this.specialTitleList)), "#".join([str(this.votemayor), str(this.candidatar), str(this.isMayor), str(this.isPresidente), str(this.votepresidente), str(this.addpresidente)]), this.playerName])
    ...
    def enterRoom(this, roomName):
        ...
        if this.privLevel >= 4:
            reactor.callLater(0.5, this.sendStaffChats)
    ...
    def sendAccountTime(this):
        ...
        else: this.Cursor.execute('update Account set Time = ? where IP = ?', [eventTime_, this.ipAddress])
    ...
    def checkVip(this, vipTime):
        ...
        if days <= 0:
            ...
            this.sendClientMessage("O seu VIP esgotou-se. Você não poderá mais usar os sistemas e comandos exclusivos para VIP's.")
            Cursor.execute("update Users set VipTime = 0 where Username = ?", [this.playerName])
        else: this.sendClientMessage("Você ainda tem <V>"+str(days)+"<BL> dias de privilégio VIP!")
    ...
    def sendContagem(this):
        this.games.RebootTimer1 = reactor.callLater(2, this.sendContRoom, "http://www.transforvida.com.br/images/icones/3.png")
        this.games.RebootTimer2 = reactor.callLater(3, this.sendContRoom, "http://www.transforvida.com.br/images/icones/2.png")
        this.games.RebootTimer3 = reactor.callLater(4, this.sendContRoom, "http://www.transforvida.com.br/images/icones/1.png")
        this.games.RebootTimer4 = reactor.callLater(5, this.sendContRoom, "http://www.transforvida.com.br/images/icones/go.png")
    ...
    def sendContRoom(this, value):
        if value == "GO":
            if this.room.isDeathmatch and not this.isDead:
                this.sendMessage("<ROSE>[DEATH]<N> GO! Mate seus adversários e seja o único sobrevivente!")
                this.countP = 0
                this.ClientGotHole = 1
                this.isDead = False
    ...
    def sendDeathInventory(this, page=1):
        ...
        message3 = ""
        message4 = ""
        message5 = '<font color="#9ab7c6"><a href="event:next">Avançar</a></font>'
        message6 = ""
        message7 = ""
        message8 = '<p align="center"><font size="28" face="Soopafresh,Verdana,Arial,sans-serif" color="#9ab7c6">Inventário</font></p>'
    ...
    def startPlay(this):
        ...
        else: shamanCode = this.room.getShamanCode()
    ...
    def sendLogin(this):
        this.sendPacket(Identifiers.send.Player_Identification, ByteArray().writeInt(this.playerID).writeUTF(this.playerName).writeInt(6000000).writeByte(this.langueID).writeInt(this.playerCode).writeByte(this.playerCode).writeBoolean(True).writeByte(this.privLevel).writeBoolean(False).toByteArray())
        if this.isGuest:
    ...
    def sendChangeMap(this, time):
       ...
       if this.room.changeMapTimer:
           try: this.room.changeMapTimer.cancel()
           except: this.room.changeMapTimer=None
    ...
    def deathRoundWinner(this, playername):
        for client in this.room.clients.values():
            ...
            this.PlayerDeathVivo = False
            client.sendMBox("<p align='center'><CH>" + str(this.playerName) + "<N> é o vencedor!</p>", 585, 30, 200, 20, "45%", "000000", "222222", 65)
            client.sendMessage("<ROSE>[DEATH] <CH>"+str(this.playerName)+"<J> foi o único sobrevivente e ganhou a partida recebendo <ROSE>+1 <J>ponto e <ROSE>7<J> firsts e moedas.")
            client.sendTitleMessage('<VP>Próxima rodada começará em 5 segundos.')
            reactor.callLater(1, client.sendTitleMessage, '<VP>Próxima rodada começará em 4 segundos.')
            reactor.callLater(2, client.sendTitleMessage, '<VP>Próxima rodada começará em 3 segundos.')
            reactor.callLater(3, client.sendTitleMessage, '<VP>Próxima rodada começará em 2 segundos.')
            reactor.callLater(4, client.sendTitleMessage, '<VP>Próxima rodada começará em 1 segundos.')
    ...
    def sendTitleMessage(this, message):
        for client in this.room.clients.values():
            info = struct.pack('!h', len(message)) + message + '\x0a'
            ...
            if int(this.room.getPlayerCount()) >= int(this.server.needToFirst) and this.room.countStats and not this.isShaman:
                if this.room.isNormRoom or this.room.isRacing:
                    if this.room.numGetCheese == 1:
                        this.XDCoins += 2
                        for client in this.room.clients.values():
                            client.sendMessage('<font color="#87CEFA">'+this.playerName+'</font> foi o primeiro a coletar o queijo e recebeu +2 moedas !')
        this.room.canChangeMap = True

    def playerWin(this, holeType, distance=-1):
        if this.activeArtefact == 0 and this.room.isEventoAventura:
            this.sendMessage("<CH>Você precisa coletar o item do evento para poder firstar. Você morreu!")
        else:
                    ...
                    if place == 1:
                        ...
                        if this.room.getPlayerCountUnique() >= this.server.needToFirst and this.room.countStats and not this.isShaman and not this.canShamanRespawn:
                            if not this.room.isEventoAventura:
                                ...
                                this.XDCoins += 3
                                for client in this.room.clients.values():
                                    client.sendMessage("<N><b>1°</b> – <N>Parabéns à <J>"+this.playerName+"<N>, firstou e recebeu <J>+4 <N>firsts e moedas.")
                            if this.room.isEventoAventura:
                                if this.activeArtefact == 1:
                                    ...
                                    this.sendSettime(20)
                                    for client in this.room.clients.values():
                                        client.sendMessage("<N><b>1°</b> – <VP>Parabéns à <CH>"+this.playerName+"<VP>, firstou no evento e recebeu <CH>+7 <VP>firsts e moedas.")

                            if this.room.isRacing:
                                if not this.isGuest:
                                    for client in this.room.clients.values():
                                        client.sendMessage("<ROSE>Trocando de mapa em 15 segundos.")
                                        this.sendSettime(15)
                            if this.room.isTribewar:
                                tribes = {}
                                for client in this.room.clients.values():
                                    if client.tribeName != '':
                                        try: tribes[client.tribeName] += 1
                                        except: tribes[client.tribeName] = 1
                                if len(tribes) >= 2:
                                    if this.tribeName != '':
                                ...
                                else: this.sendMessage('<V>[TribeBOT] Precisa ter mais de uma Tribo participante para valer pontos!')
                                    ...
                                    if not this.room.mapCode in [-1,31,41,42,54,55,59,60,62,89,92,99,114,801]:
                                        try:
                                            ...
                                            if timeDB[0] == 0 or timeTaken < timeDB[0]:
                                                ...
                                                this.sendMessage("<R>Parabéns, você bateu o recorde de velocidade nesse mapa! Tempo decorrido recorde foi de: <J>"+str(t)+"s"+"<R>. Você recebeu: <J>5<R> moedas por isso.")

                                                for client in this.room.clients.values():
                                                    client.sendMessage("<N>Vuashh! O Jogador <J>"+this.playerName+" <N>bateu o recorde nesse mapa <J>@"+str(this.room.mapCode)+"<N> com o melhor tempo de <J>"+str(t)+"s<N> e ganhou <J>5<N> moedas.")
                                        except: pass
                        ...
                        if this.room.getPlayerCountUnique() >= this.server.needToFirst and this.room.countStats and not this.isShaman and not this.canShamanRespawn:
                            if not this.room.isEventoAventura:
                                ...
                                this.cheeseCount += 2
                                this.XDCoins += 2
                                this.sendMessage("<N><b>2°</b> – <N>Parabéns <J>"+this.playerName+"<N>, entrou em segundo lugar e recebeu <J>+3 <N>firsts e moedas.")
                            if this.room.isEventoAventura:
                                if this.activeArtefact == 1:
                                    this.firstCount += 3
                                    this.cheeseCount += 3
                                    this.XDCoins += 3
                                    this.aventurePoints[24] += 1
                                    this.sendMessage("<N><b>2°</b> – <VP>Parabéns <CH>"+this.playerName+"<VP>, entrou em segundo lugar no evento e recebeu <CH>+4 <VP>firsts e moedas.")
                        ...
                        if this.room.getPlayerCountUnique() >= this.server.needToFirst and this.room.countStats and not this.isShaman and not this.canShamanRespawn:
                            if not this.room.isEventoAventura:
                                ...
                                this.cheeseCount += 2
                                this.XDCoins += 2
                                this.sendMessage("<N><b>3°</b> – <N>Parabéns <J>"+this.playerName+"<N>, entrou em terceiro lugar e recebeu <J>+2 <N>firsts e moedas.")
                            if this.room.isEventoAventura:
                                if this.activeArtefact == 1:
                                    this.firstCount += 1
                                    this.cheeseCount += 1
                                    this.XDCoins += 1
                                    this.aventurePoints[24] += 1
                                    this.sendMessage("<N><b>3°</b> – <VP>Parabéns <CH>"+this.playerName+"<VP>, entrou em terceiro lugar no evento e recebeu <CH>+2 <VP>firsts e moedas.")
                                    
                        this.playerScore += (2 if this.room.isRacing else 12) if not this.room.noAutoScore else 0
                    else: this.playerScore += (1 if this.room.isRacing else 10) if not this.room.noAutoScore else 0
    ...
    def sendPlayerWin(this, place, timeTaken):
        ...
        if not this.cheeseCount >= 60000 and not this.room.isEditor and not this.room.EMapValidated and not this.room.EMapCode:
            if timeTaken < 500:
                if this.privLevel <= 3:
                    this.sendPlayerBan(0, "Autowin Detectado - Se persistir, você será banido.", True)
                    this.transport.loseConnection()
                    this.server.disconnectIPAddress(this.ipAddress)
    ...
    def sendProfile(this, playerName):
        ...
        if player != None and not player.isGuest:
            packet = ByteArray().writeUTF(player.playerName).writeInt(this.server.getplayerAvatar(playerName)).writeInt(str(player.regDate)[:10]).writeByte({1:1, 2:1, 3:13, 4:13, 5:11, 6:11, 7:5, 8:5, 9:5, 10:10}[player.privLevel]).writeByte(player.gender).writeUTF(player.tribeName).writeUTF(player.marriage) #.writeInt(0).writeInt(str(player.regDate)[:10]).writeInt(0).writeShort(0)
    ...
    def sendPlayerBan(this, hours, reason, silent):
        ...
        if not silent and this.room != None:
            for client in this.room.clients.values():
                client.sendLangueMessage("", "<ROSE>• [Moderação] $Message_Ban", this.playerName, str(hours), reason)
        this.server.disconnectIPAddress(this.ipAddress)
    ...
    def sendPlaceObject(this, objectID, code, px, py, angle, vx, vy, dur, sendAll):
        ...
        if this.isGuest or sendAll:
            packet.writeByte(0)
        else: packet.writeBytes(this.parseShop.getShamanItemCustom(code))

        if not sendAll:
            this.room.sendAllOthers(this, Identifiers.send.Spawn_Object, packet.toByteArray())
            this.room.objectID = objectID
        else: this.room.sendAll(Identifiers.send.Spawn_Object, packet.toByteArray())
    ...
    def sendModMute(this, playerName, hours, reason, only):
        if not only:
            ...
            if client:
                client.sendLangueMessage("", "<ROSE>• [Moderação] $MuteInfo1", hours, reason)
    ...
    def sendGameMode(this, mode):
        try:
            ...
            if not modeInfo[0] == "":
                ...
                minigamesList = {}
                minigames = ["#eventoaventura", "#fly", "#ballonrace", "#invocation", "#explosion", "#pokelua", "#deathmatch", "#tribewar"]
                roomsList = {}
        ...
        except: this.transport.loseConnection()
...
class Server(protocol.ServerFactory):
    ...
    def __init__(this):
        ...
        # List
        this.packetKeys = [0, 26, 37, 36, 24, 59, 22, 35, 56, 34, 121, 120, 68, 103, 114, 71, 92, 70, 113, 112]
        this.loginKeys = [5204292, 13207023, 1024, 2097152, 1024, 2543425, 10182091, 16280609, 16384, 67108864, 13037573, 8858369, 2597112]
        this.tempIPBanList = []
        this.userMuteCache = []
        this.shopPromotions = []
        this.IPTempBanCache = []
        ...
        this.statsclient = {"racingCount":[1500,10000,10000,10000], "survivorCount":[1000,800,20000,10000], "racingBadges":[124,125,126,127], "survivorBadges":[120,121,122,123]}
    ...
    def checkMessage(this, client, message):
        ...
        while i < len(blackList):
            if re.search("[^a-zA-Z]*".join(blackList[i]), message.lower()):
                this.sendStaffMessage(7, "[<V>ANTI-DV</V>][<J>%s</J>][<V>%s</V>][<R>%s</R>] Está enviando um link da blacklist, portanto esta mensagem não aparece para outros jogadores." %(client.ipAddress, client.playerName, message))
                isSuspect = True
                return True
            i += 1
        if not isSuspect:
    ...
    def banPlayer(this, playerName, bantime, reason, modName, silent):        
        ...
        if client != None:
            ...
            if not modName == "Server":
                ...
                Cursor.execute("insert into BanLog values (?, ?, ?, ?, ?, 'Online', ?)", [playerName, modName, bantime, reason, int(time.time() / 10), client.ipAddress])
            else: this.sendStaffMessage(5, "<V>Servidor <BL>baniu o jogador <V>%s<BL> por <V>1 <BL> hora. Motivo: <V>Vote Populaire<BL>." %(playerName))
    ...
    def voteBanPopulaire(this, playerName, playerVoted, ip):
        ...
        if client != None and client.privLevel == 1 and not ip in client.voteBan:
            ...
            this.sendStaffMessage(7, "O jogador <V>%s</V> está votando contra <V>%s</V> [<R>%s</R>/10]" %(playerVoted, playerName, len(client.voteBan)))
    ...
    def getModMuteInfo(this, playerName):
        ...
        if rs:
            return [rs["Reason"], rs["Time"]]
        else: return ["Without a reason", 0]
    ...
    def sendStaffChat(this, type, langue, playerName, message, sender):
        playerName = sender.playerName if type == -1 else "" if type == 0 else "Message Serveur" if type == 1 else sender.langue.upper() + "][" +({11:"Admin][", 10:"Admin][", 9:"Coord][", 8:"Smod][", 7:"Mod][", 6:"MapCrew][", 5:"Helper][", 4:"DV][", 3:"LUA]["}[sender.privLevel])
    ...
    def getPlayersCountMode(this, mode, langue):
        modeName = "TransforVida - Geral" if mode == 1 else "TransforVida vanilla" if mode == 3 else "TransforVida survivor" if mode == 8 else "TransforVida racing" if mode == 9 else "Transformice music" if mode == 11 else "TransforVida bootcamp" if mode == 2 else "TransforVida defilante" if mode == 10 else "TransforVida village" if mode == 16 else "" if mode == 18 else "TransforVida Module"
        playerCount = 0
    ...
    def setVip(this, playerName, days):
        ...
        if client != None:
            if client.privLevel == 1:
                ...
                client.privLevel = 2
                this.sendModMessage(7, "<V>"+playerName+"<BL> se tornou <ROSE>VIP <BL>do <ROSE>TransforVida<BL> por <V>"+str(days)+"<BL> dias.")
        else:
            Cursor.execute("update Users SET VipTime = ?, PrivLevel = 2 WHERE Username = ?", [Utils.getTime() + ((24 * days) * 3600), playerName])
            Cursor.fetchall()
            this.sendModMessage(7, "<V>"+playerName+"<BL> se tornou VIP do <ROSE>TransforVida<BL> por <V>"+str(days)+"<BL> dias.")

    def getPlayerCode(this, playerName):
        client = this.players.get(Utils.parsePlayerName(playerName))
        return client.playerCode if client != None else 0
...
class Room:
        ...
        if this.initVotingMode:
            ...
            if this.catchTheCheeseMap:
                ...
                if client != None:
                    ...
                    if not this.noAutoScore: client.playerScore = numCom
                    if numCom > 0:
            ...
            if not this.mapCode in [-1,31,41,42,54,55,59,60,62,89,92,99,114,801]:
                ...
                try:
                    if rs[0] > 0:
                        if rs[0] > 100:
                            ...
                            t = (rs[0]/10.0)
                        for client in this.clients.values():
                            client.sendMessage("<BL>Um novo mapa foi iniciado <J>@"+str(this.mapCode)+"<BL>. O melhor tempo nesse mapa foi de <J>"+str(t)+"s<BL> pelo jogador <J>"+str(rs[1])+"<BL>.")
                except: pass
            ...
            if this.getPlayerCount() >= this.server.needToFirst:
                if this.isEventoAventura:
                    ...
                    anyone = 1
                    itemID = random.choice([7462, 7463, 7464, 7466, 7467])
    ...
    def sendAllChat(this, playerCode, playerName, message, langueID, isOnly):
        ...
        if not isOnly:
            for client in this.clients.values():
                if not playerName in client.ignoredsList:
...
if __name__ == "__main__":
    ...
    os.system("title Source TransforVida - 1.380")
    validPorts = []
    S = Server()
    for port in [443, 6112, 5555, 44440, 44444, 3724]:
        try:
            reactor.listenTCP(port, S)
            validPorts.append(port)
        except: print "Portas falharam > " + str(port)
